...
| CPU | 重复发起单次测量并收集结果；首次校准对 10 个结果求平均，运行期按 1 s 采样间隔累计 3 个结果求平均，再按原文档的 `t/T` 区间选择或保持采样沿。 |

CPLD 每次事务只发布一个可读结果，不在 RTL 中做多样本平均。多次测量的调度、平均和采样沿裁定均属于 CPU 软件。
...
7. CPU 可再次发出 `start` 以收集后续样本；若旧结果未读，新事务开始时先使旧结果无效，新结果完成后再覆盖旧数值并重新置位 `result_valid`。CPU 将每次读取的数值收入当前 10 样本或 3 样本窗口。
...
CPU 校准策略不属于 CPLD RTL，但属于原系统闭环：上电或频率变化后收集 10 个结果求平均并建立初始采样沿；运行期以 1 s 为采样间隔，累计 3 个结果求平均后执行持续校准。本文档采用“3 样本非重叠批次”作为重建裁决；原资料未明确首个运行期样本的启动时点。
...
4. 输入最小高低电平与最小有效跳变间隔。
5. CPU 如何检测频率变化并重新进入 10 样本首次校准。

## 13. CPU 参考增强架构

CPU 参考模型在既有 CPLD 寄存器接口之后增加纯软件处理层：多次读取单次结果，执行圆周统计，检查集中度与归一化决策裕量，再决定配置、保持或重测。该层不增加 CPLD 状态，不改变 `phase_result`、`result_valid` 或读清协议。

历史普通平均和候选增强并列保留，便于回归比较。候选门限尚未冻结，因此当前架构结论是“接口足够支持算法探索”，不是“候选算法已成为产品实现”。
