...
- 原资料未定义 $4T/12 \le t \le 5T/12$、$7T/12 \le t \le 8T/12$ 及所有阈值等号的归属；
...
候选增强中，首次校准置信不足时不发布新采样沿并请求重测；运行期置信不足时保持当前沿。探索门限为：集中度 $\ge 0.9$、圆周裕量与 $T$ 的比值 $\ge 0.025$ 仅用于场景比较，不构成产品要求。完整证据和恢复条件见 `cpu_algorithm_exploration_zh-CN.md`。
