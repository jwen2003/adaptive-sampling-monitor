...
v1.0 以已完成 RTL 为准：CPU 通过 `mem13[3]` 写 `1→0`，寄存器接口生成单拍 `measurement_start`；启动时清除旧结束标志，CPU 等待新结束标志并由外部 `cpu_read_clear` 读清。CPU 不对多次结果求平均；区间裁定不进入 CPLD 核心 RTL。
...
- 复位为同步复位；
- 忠实版没有无时钟或无数据翻转超时；
- CPU 逐结果按 `t/T` 区间裁定，不做平均。

## 3. 剩余待确认项

1. 原寄存器地址和总线读时序；
2. 目标器件型号及同步器属性；
3. 若实现 CPU 参考程序，特殊区间空挡采用最近区间还是保持上一选择。
...
CPU 参考模型不需要验证平均算法。应验证：

1. 单个 `phase_result` 正确换算为 `t`；
2. 使用当前周期 `T` 选择正确的普通或特殊区间；
3. 区间内结果产生确定的边沿选择；
4. 特殊空挡使用选定的一致规则；
5. 空挡两侧不会出现未定义返回值。

若空挡采用“最近区间”，测试最近距离和正中点的平局规则；若采用“保持上一选择”，测试初始默认值和连续进入空挡的行为。
...
| `event_detector_tb.sv` | `PASS`，15 项检查 | 首样本建立基线、上升/下降/翻转、单拍输出；最终接口不含 `sync_valid`。 |
| `phase_measurement_tb.sv` | `PASS`，12 项检查 | 启动、等待原点、结果 1与3、最近原点替换、同拍0、读清、重启与复位。 |
| `register_interface_tb.sv` | `PASS`，16 项检查 | 写1武装、写0启动、忙时拒绝、结果映射与同步复位。 |
| `adaptive_sampling_monitor_tb.sv` | `PASS`，29 项检查，退出码 0 | 异步输入到寄存器的端到端路径、相位1、同拍0、忙时写拒绝、读清与最终复位。 |
...
`cpu_decision_model.py` 尚未实现，不列入 v1.0 已交付文件。
